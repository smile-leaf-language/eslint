    ...
    create(context) {
        ...
        */
        function addElementListIndent(tokens, elements, offset) {
            ...
            /**
            * Gets the first token of an element at the given index, including surrounding parentheses.
            * @param {number} elementIndex The index of the element in `elements`
            * @returns {Token} The first token of this element
            */
            function getFirstTokenByElementsIndex(elementIndex) {
                let token = sourceCode.getTokenBefore(sourceCode.getFirstToken(elements[elementIndex]));
            ...
            if (offset === "first" && elements.length) {
                ignoreToken(getFirstTokenByElementsIndex(0));
            }
            ...
            elements.slice(1).forEach((element, index) => {
                if (offset === "first") {
                    ...
                    // This element is at `index + 1` due to .slice(1) index offsetFrom
                    setDesiredOffset(getFirstTokenByElementsIndex(index + 1), null, getFirstTokenByElementsIndex(0).loc.start.column / indentSize);
                } else {
                    const previousElement = elements[index]; // The previous element is at `index` due to .slice(1) index offset
                    const firstTokenOfPreviousElement = previousElement && getFirstTokenByElementsIndex(index);

                    if (previousElement && previousElement.loc.end.line > tokens[0].loc.end.line) {
                        matchIndentOf(firstTokenOfPreviousElement, getFirstTokenByElementsIndex(index + 1));
                    }
                }
            });
        ...
        */
        function addFunctionCallIndent(node) {
            const openingParen = node.arguments.length ? sourceCode.getTokensBetween(node.callee, node.arguments[0]).find(token => token.value === "(") : sourceCode.getLastToken(node, 1);
        ...
        */
        function addParensIndent(tokens) {
            ...
            parenPairs.forEach(pair => {
                ...
                if (!parameterParens.has(leftParen) && !parameterParens.has(rightParen) && sourceCode.getTokenAfter(leftParen).loc.start.line > leftParen.loc.start.line) {
